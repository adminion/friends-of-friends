...
module.exports = function friendshipInit(mongoose, options) {
    ...
    }, { collection: options.friendshipCollectionName });
    ...
    try {
        model = mongoose.model(options.friendshipModelName, FriendshipSchema);
    } catch (error) {
        model = mongoose.model(options.friendshipModelName);
    }
