...
module.exports = function pluginInit (mongoose, options) {

    return function friendshipPlugin (schema, options) {
        ...
        Friendship = require('./friendship')(mongoose, options)
        ...
        schema.statics.friendRequest = function (requesterId, requestedId, done) {
            debug('AccountModel.friendRequest');
        ...
        schema.statics.getRequests = function (accountId, done) {
            debug('AccountModel.getRequests')
            var accountModel = mongoose.model(options.accountName);
        ...
        schema.statics.getSentRequests = function (accountId, done) {
            debug('AccountModel.getSentRequests')
        ...
        schema.statics.getReceivedRequests = function (accountId, done) {
            debug('AccountModel.getReceivedRequests')
        ...
        schema.statics.acceptRequest = function (requesterId, requestedId, done) {
            debug('AccountModel.acceptRequest')
         ...
         * @function    AccountModel.cancelRequest
         * @param       {ObjectId} requesterId  - the _id of the requester of friendship
         * @param       {ObjectId} requestedId  - the _id of the user whose friendship was requested
        ...
        schema.statics.getFriendsOfFriends = function (accountId, done) {
            debug('AccountModel.getFriendsOfFriends')
        ...
        schema.statics.getNonFriends = function (accountId, done) {
            debug('AccountModel.getNonFriends');
            ...
            async.parallel({
                friends: function (complete) {
                    Friendship.getFriends(accountId, complete);
                },
                friendsOfFriends: function (complete) {
                    Friendship.getFriendsOfFriends(accountId, complete);
                },
                pendingFriends: function (complete) {
                    Friendship.getPendingFriends(accountId, complete);
                }
            }, function (err, results) {

                debug('friendIds of ' + accountId, results.friendIds)
                debug('friendIds of ' + accountId + '\'s friends', results.friendIdsOfFriends)
                debug('Ids of ' + accountId + '\'s friends', results.pendingFriends)

                var conditions = {
                    '_id': { 
                        '$ne' : accountId,
                        '$nin': results.friends.concat(results.friendsOfFriends.concat(results.pendingFriends))
                    }
                };
        ...
        schema.statics.arePendingFriends = function (accountId1, accountId2, done) {
            debug('AccountModel.arePendingFriends')
        ...
        schema.statics.getFriendship = function (accountId1, accountId2, done) {
            debug('AccountModel.getFriendship')
        ...
        schema.statics.getRelationship = function (accountId1, accountId2, done) {
            debug('AccountModel.getRelationship');
        ...
        schema.statics.isRequester = function (friendshipId, accountId, done) {
            debug('AccountModel.isRequester');
        ...
        schema.statics.isRequested = function (friendshipId, accountId, done) {
            debug('AccountModel.isRequested');
        ...
        schema.methods.friendRequest = function (requestedId, done) {
            debug('AccountDocument.friendRequest')
        ...
        schema.methods.getRequests = function (done) {
            debug('AccountDocument.getRequests');
        ...
        schema.methods.getSentRequests = function (done) {
            debug('AccountDocument.getSentRequests');
        ...
        schema.methods.getReceivedRequests = function (done) {
            debug('AccountDocument.getReceivedRequests');
        ...
        schema.methods.acceptRequest = function (requesterId, done) {
            debug('AccountDocument.acceptRequest');
            this.constructor.acceptRequest(requesterId, this._id, done);
        };

        /**
         *  cancel a friend request sent to the specified user
         * @function    AccountDocument.cancelRequest
         * @param       {ObjectId} requestedId  - the _id of the account to whom the request was sent
         * @param       {Function} done         - required callback, passed the populated request that was denied
         * @see         [AccountModel.cancelRequest]{@link AccountModel.cancelRequest}
         */
        schema.methods.cancelRequest = function (requestedId, done) {
            debug('AccountDocument.cancelRequest');
        ...
        schema.methods.denyRequest = function (requesterId, done) {
            debug('AccountDocument.denyRequest');
        ...
        schema.methods.endFriendship = function (accountId, done) {
            debug('AccountDocument.endFriendship');
        ...
        schema.methods.getFriends = function (done) {
            debug('AccountDocument.getFriends');
        ...
        schema.methods.getFriendsOfFriends = function (done) {
            debug('AccountDocument.getFriendsOfFriends');
        ...
        schema.methods.getPendingFriends = function (done) {
            debug('AccountDocument.getPendingFriends');
        ...
        schema.methods.getNonFriends = function (done) {
            debug('AccountDocument.getNonFriends');
        ...
        schema.methods.isFriend = function (accountId, done) {
            debug('AccountDocument.isFriend');
        ...
        schema.methods.isFriendOfFriends = function (accountId, done) {
            debug('AccountDocument.isFriendOfFriends');
        ...
        schema.methods.isPendingFriend = function (accountId, done) {
            debug('AccountDocument.isPendingFriend');
        ...
        schema.methods.getFriendship = function (accountId, done) {
            debug('AccountDocument.getFriendship');
        ...
        schema.methods.getRelationship = function (accountId, done) {
            debug('AccountDocument.getRelationship');
        ...
        schema.methods.isRequester = function (friendshipId, done) {
            debug('AccountDocument.isRequester');
        ...
        schema.methods.isRequested = function (friendshipId, done) {
            debug('AccountDocument.isRequested');
