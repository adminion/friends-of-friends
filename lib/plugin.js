...
module.exports = function pluginInit(mongoose) {
    ...
    return function friendshipPlugin(schema, pluginOptions) {
         ...
         * @function    PersonModel.getCommonFriends
         * @param       {ObjectId} personId1   - the _id of person1
         * @param       {ObjectId} personId2   - the _id of person2
         * @param       {Function} done         - required callback, passed err and a Relationship value
         * @see         [FriendshipModel.getCommonFriends]{@link FriendshipModel.getCommonFriends}
         */
        schema.statics.getCommonFriends = function (personId1, personId2, done) {
            debug('PersonModel.getCommonFriends');

            var personModel = mongoose.model(pluginOptions.personModelName);

            Friendship.getCommonFriends(personId1, personId2, function (err, friendIds) {
                if (err) {
                    done(err);
                } else {
                    var conditions = {
                    ...
                    personModel.find(conditions, done);
                }
            })
        };
        ...
        /**
         *  get common friends of this document's friends
         * @function    PersonDocument.getCommonFriends
         * @param       {ObjectId} personId    - the _id of the user to check for friendship
         * @param       {Function} done         - required callback, passed a boolean determination
         * @see         [PersonModel.getCommonFriends]{@link PersonModel.getCommonFriends}
         */
        schema.methods.getCommonFriends = function (personId, done) {
            debug('PersonDocument.getCommonFriends');
            this.constructor.getCommonFriends(this._id, personId, done);
        };
